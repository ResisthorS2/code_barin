...
Dialog::Dialog(QWidget *parent) // constructeur du Dialog
    : QDialog(parent)
    , ui(new Ui::Dialog)
{
    ui->setupUi(this);
...
void Dialog::on_pushButtonMain_clicked() // lorsqu'on apppuie sur le bouton Main, ça ferme le Dialog de Serge
{
    Dialog::close();
}

void Dialog::showMission(int number) // fonction qui affiche la mission du personnage selon le no de mission
{
    // changement du texte dans le label selon le no de mission
    switch (number) {
    case 0:
